...
class KubeT implements Serializable {
  def steps
  ...
  */
  def klock(String method, String owner) {
    ...
    kube {
      return sh( script: "bash ${this.cloudAutomationPath}/gen3/bin/klock.sh ${method} jenkins ${owner} 3600 -w 60", returnStatus: true)
    }
  }
  ...
  */
  def editManifest(String serviceName, String quayBranchName=null, String manifestPath="cdis-manifest") {
    ...
    if (null == quayBranchName) {
      branchName = this.conf.BRANCH_FORMATTED
    }
  ...
  */
  def selectAndLockNamespace(List<String> namespaces, String owner) {
    if (null == namespaces) {
      namespaces = this.conf.namespaces
    }
    ...
    for (int i=0; i < namespaces.size() && lockStatus != 0; ++i) {
      ...
      println "selected namespace ${this.kubectlNamespace} on executor ${env.EXECUTOR_NUMBER}"
